...
| Identity is preserved better by masked steering. | Every parent face-similarity interval spanned zero. In replication, both masked-versus-unmasked intervals spanned zero, and no face contrast survived the broader 16-test Holm family. | Preregistered primary hypothesis not supported in either campaign. | Larger, higher-coverage replication and human identity judgments; do not promote from current evidence. |
