...
\title{Evaluating Skin-Tone Control and Image Preservation in SDXL}
...
\begin{abstract}
Controlling apparent skin tone in generated portraits can unintentionally
alter apparent identity, pose, or background, yet preservation comparisons are
unreliable when methods produce different amounts of target change. We
evaluate whether a linear direction derived from paired portrait latents can
control apparent skin tone in Stable Diffusion XL while preserving other image
...
only when they achieve similar measured image-colour changes and with detector
...
denoising-time intervention. Second, we provide a prospectively frozen matched-change
...
Race and ethnicity are socially constructed and multidimensional, not fixed
one-dimensional visual attributes \cite{hanna2020critical}. The intervention in this study is therefore called a
\emph{skin-tone direction}. It must not be interpreted as changing, estimating,
or classifying race. Even datasets designed to balance face categories must
operationalize a limited set of visual labels, and report performance
consequences from category imbalance \cite{karkkainen2021fairface}. The present method is intended for controlled
...
\end{equation}
All runs use SDXL base 1.0 revision 462165984030 at
$1024\!\times\!1024$ resolution. The implementation uses 25 DPM-Solver++
multistep updates \cite{lu2022dpmsolverpp} with classifier-free guidance 7.5.
...
stepwise injection. The frozen configuration contains 30 held-out
...
Thus, the prespecified three-degree results should not be interpreted as
...
measurements. Face-recognition systems can exhibit demographic performance
differentials \cite{grother2019frvt}; the embedding model, face detector, and
cheek heuristic used here were not validated on real demographic groups.
Matched-change coverage is incomplete and particularly low for
...
or library revisions. The study plan and decision scripts were frozen in
author-controlled, content-hashed archives before the relevant outcome reviews,
but were not deposited with an independent timestamping service. We therefore
describe the design as prospectively frozen rather than externally preregistered.
...
test narrowly missed .05. The prespecified face-similarity hypothesis was not
